...
class BiModal(Image, abc.ABC):
    ...
    def __init__(self, data: npt.NDArray[np.int_], exp_bg: np.int_, exp_fg: np.int_, *,
                 static_range: _tuple[int, int] = None):
        self._bg, self._fg = exp_bg, exp_fg
        super().__init__(data, static_range=static_range)
        if self._min is not None and self._max is not None:
            if self._bg != self._min or self._bg != self._max or self._fg != self._min or self._fg != self._max:
                raise ValueError("Static range should be the minima and maxima in bimodal images")
...
class RGB(Image, abc.ABC):
    ...
    def channel(self, channel: Channel) -> npt.NDArray[np.int_]:
        ...
        if channel == Channel.R:
            mask = (normalised >= 0) & (normalised <= 2 ** 8 - 1)
        elif channel == Channel.G:
            mask = (normalised >= 2 ** 8) & (normalised <= 2 ** 16 - 1)
        else:
            mask = (normalised >= 2 ** 16) & (normalised <= 2 ** 24 - 1)
        normalised = data[np.nonzero(mask)]
        return normalised
    ...
    def make_grey(self, strength: float) -> np.int_:
        ...
        """
        return self.make_red(strength, strength)
