...
from ... import utils
from ..._base import CanvasPage, SettingsPage, images
from ..._errors import *
...
class Order(utils.SettingsPopup):
    ...
    def __init__(self):
        ...
        def _num(init: int, pipe: validation.Pipeline = validation.examples.kernel) -> utils.Spinbox:
            return utils.Spinbox(init, 1, pipe)
        ...
        def _morph(name: str) -> utils.PALFunction:
            return utils.PALFunction(name,
                                     ...
                                     Width=_num(5),
                                     Shape=_transform(),
                                     Multiplier=_num(1, validation.examples.morph),
                                     Repeats=_num(1, validation.examples.morph),
                                     )
        ...
        self._gss_blur = utils.PALFunction("gss_blur", True,
                                           Height=_num(5),
                                           Width=_num(5),
                                           Sigma_X=_num(0, validation.examples.sigma),
                                           Sigma_Y=_num(0, validation.examples.sigma)
                                           )
        self._sharpen = utils.PALFunction("sharpen",
                                          Size=_num(5),
                                          Scale=_num(1, validation.examples.sigma),
                                          Delta=_num(0, validation.examples.sigma)
                                          )
