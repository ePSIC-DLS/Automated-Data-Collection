...
__all__ = ["FileDialog", "FilePrompt", "Canvas"]
...
class Canvas(widgets.QWidget):
    ...
    def histogram(self, image: GreyImage, groups=15, colour=RGB(0, 0, 255)):
        """
        ...
        for grey, x, height in zip(greys, np.arange(0, w - adj_step, adj_step), heights):
            if height == 0:
                continue
            blank.drawing.rect.from_size((x, w - 1, AABBCorner.BOTTOM_LEFT), (max(adj_step - 2, 2), height), colour,
                                         fill=Grey(grey))
